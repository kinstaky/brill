...
int main(int argc, char **argv) {
	...
	if (glimmer::ParseProgramOptions(argc, argv, options, true, true) != 0 || options.help) {
		glimmer::PrintGeneralUsage(argv[0], true, true);
	...
	return glimmer::RunDssdNormalize(config, options.detector, options.trigger, options.run, options.end_run) == 0 ? 0 : 1;
}
