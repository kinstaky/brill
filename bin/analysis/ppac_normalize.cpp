...
int main(int argc, char **argv) {
	...
	if (glimmer::ParseProgramOptions(argc, argv, options, false, true) != 0 || options.help) {
		glimmer::PrintGeneralUsage(argv[0], false, true);
	...
	return glimmer::RunPpacNormalize(config, options.trigger, options.run, options.end_run) == 0 ? 0 : 1;
}
